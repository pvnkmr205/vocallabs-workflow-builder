...
A multi-tenant, event-driven execution engine for chaining AI agent steps, built with Next.js, Nhost (Hasura/PostgreSQL), and serverless Node.js functions.
...
2. **Live Subscription:** The Next.js frontend uses an Apollo GraphQL subscription to watch the `step_runs` table. When the status hits `paused_awaiting_approval`, the UI reactively renders the "Approve & Continue" button.
...
2. Start the local Nhost backend using the Nhost CLI: `nhost up`.
3. Apply the database metadata and migrations: `nhost metadata apply` (if not auto-applied).
4. Run the Next.js development server: `npm run dev`.
5. Access the app at `http://localhost:3000`.

*Note: As per instructions, the external LLM/HTTP calls inside the execution engine are currently stubbed using a disclosed 2-second artificial delay to simulate network latency.*